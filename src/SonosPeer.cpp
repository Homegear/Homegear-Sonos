...
std::string SonosPeer::getRinconId()
{
	try
	{
		...
		return parameter->convertFromPacket(parameterData, false, false)->stringValue;
	}
	catch(const std::exception& ex)
	{
		GD::out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
void SonosPeer::setRinconId(std::string value)
{
	try
	{
		...
		configParameter.rpcParameter->convertToPacket(PVariable(new Variable(value)), false, parameterData);
...
void SonosPeer::setRoomName(std::string value, bool broadCastEvent)
{
	try
	{
		...
		std::vector<uint8_t> parameterData;
		configParameter.rpcParameter->convertToPacket(variable, false, parameterData);
		if(configParameter.equals(parameterData)) return;
		configParameter.setBinaryData(parameterData);
		if(configParameter.databaseId > 0) saveParameter(configParameter.databaseId, parameterData);
			...
			for(std::unordered_map<std::string, FrameValue>::iterator i = a->values.begin(); i != a->values.end(); ++i)
			{
				for(std::list<uint32_t>::const_iterator j = a->paramsetChannels.begin(); j != a->paramsetChannels.end(); ++j)
				{
					...
					if(parameter.rpcParameter)
					{
						...
						PVariable value = parameter.rpcParameter->convertFromPacket(i->second.value, parameter.invert(), true);
						...
						if(i->first == "CURRENT_ALBUM_ART")
						{
                            ...
                            parameter.rpcParameter->convertToPacket(value, parameter.invert(), i->second.value);
						}
						...
						else if(i->first == "AV_TRANSPORT_URI")
						{
							...
							BaseLib::PVariable oldValue = parameter.rpcParameter->convertFromPacket(transportParameterData, parameter.invert(), true);
                            ...
                            //Update IS_MASTER and MASTER_ID
                            {
                                ...
                                if(parameter2.rpcParameter)
                                {
                                    ...
                                    {
                                        parameter2.rpcParameter->convertToPacket(isMaster, parameter2.invert(), parameterData);
                                        ...
                                        if(parameter3.rpcParameter)
                                        {
                                            ...
                                            parameter3.rpcParameter->convertToPacket(masterId, parameter3.invert(), parameterData2);
                            ...
                            //{{{ Update IS_STREAM
                                ...
                                if(parameter3.rpcParameter)
								{
									...
									if(parameterData.empty() || (bool) parameterData.back() != _isStream)
									{
										...
										parameter3.rpcParameter->convertToPacket(isStream, parameter3.invert(), parameterData);
						...
						else if(_isStream)
						{
							if(i->first == "CURRENT_TRACK_STREAM_CONTENT")
							{
								...
								if(parameter2.rpcParameter)
								{
									...
									parameter2.rpcParameter->convertToPacket(value, parameter2.invert(), binaryData);
	...
	try
	{
		if(channel == 1)
		{
			if(parameter->id == "IP_ADDRESS")
			{
				std::vector<uint8_t> parameterData;
                auto& rpcConfigurationParameter = valuesCentral[channel][parameter->id];
				parameter->convertToPacket(PVariable(new Variable(_ip)), rpcConfigurationParameter.invert(), parameterData);
...
PVariable SonosPeer::setValue(BaseLib::PRpcClientInfo clientInfo, uint32_t channel, std::string valueKey, PVariable value, bool wait)
{
	try
	{
		...
		value = rpcParameter->convertFromPacket(parameterData, parameter.invert(), true);
